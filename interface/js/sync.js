const CloudSync = {
  ...
  flushPendingUpload() {
    if (!this._uploadTimer) return;
    clearTimeout(this._uploadTimer);
    this._uploadTimer = null;
    if (!this.isSignedIn()) return;
    return this.pushToCloud({ force: true }).catch((e) => console.warn('Cloud upload failed', e));
  },
  ...
  startAutoSync() {
    ...
    if (!this._autoSyncBound) {
      this._autoSyncBound = true;
      document.addEventListener('visibilitychange', () => {
        if (!this.isSignedIn()) return;
        if (document.visibilityState === 'hidden') {
          this.flushPendingUpload();
          return;
        }
        this.syncNow().catch(() => {});
      });
      window.addEventListener('pagehide', () => {
        if (this.isSignedIn()) this.flushPendingUpload();
      });
    }
  },
