# Documentação técnica — Elevador

| Ficheiro | Conteúdo |
|----------|----------|
| [pinagem.md](pinagem.md) | Mapa de pinos Mega — simulação e maquete |
| [estados_sistema.md](estados_sistema.md) | Máquina de estados do firmware |

## Firmware

| Variante | Pasta |
|----------|-------|
| Simulação v06 | [simulacao/legacy/wokwi/v06_atual_pedidos_internos_porta/](../simulacao/legacy/wokwi/v06_atual_pedidos_internos_porta/) |
| **Maquete (activo)** | [real/codigo_final_3_0_maquete/](../real/codigo_final_3_0_maquete/) |
| Completo (congelado) | [real/codigo_final_3_0/](../real/codigo_final_3_0/) |

## Esquemas

- [ELV-SCH-001 sim](../hardware/esquemas/elevador_v06_forca_comando.svg)
- [ELV-SCH-002 real](../hardware/esquemas/elevador_real_forca_comando.svg)

## Navegação

| | |
|--|--|
| Elevador | [../README.md](../README.md) |
| Testes bancada | [../testes/bancada/](../testes/bancada/) |
