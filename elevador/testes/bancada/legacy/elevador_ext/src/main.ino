/*
  ELEVADOR 4 PISOS — L298N + pedidos EXTERIORES (sem OLED, sem botões interiores)
  Baseado no firmware real maquete — lógica SCAN, buzzer, reset/rearme, erro.

  Pinagem Mega (bancada):
    Botões ext: D2-D5   LEDs ext: A0-A3
    Hall piso 1-4: D7, D8, D11, D12  (LOW = íman)
    Porta (chave): D22  (LOW = fechada)
    Reset: D10
    Buzzer: D6
...
void lerSensoresPiso() {
  for (int i = 0; i < 4; i++) {
    estadoSensor[i] = (digitalRead(sensoresHall[i]) == LOW);
  }
}
...
void setup() {
  ...
  Serial.println(F("=== ELEVADOR EXT (sem OLED/int) ==="));
  ...
  if (andar == -1) {
    Serial.println(F("Referencia: desce ate piso 1..."));
    ...
    if (andar == -1) {
      estadoSistema = ERRO_PARADO;
      iniciaAlarmeContinuo();
      Serial.println(F("ERRO: referencia piso 1"));
    } else {
      Serial.print(F("Referencia OK — piso "));
      Serial.println(andar);
    }
  } else {
    Serial.print(F("Arranque no piso "));
    Serial.println(andar);
  }
